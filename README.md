...
The “[Dissertation.pdf](https://github.com/costa4487/Dissertation-2015/blob/master/Dissertation.pdf)” document provides a full explanation of the research, development, and implementation of this project.
...
2. Controller: Analyze network traffic matrix and determine optimal topology
    ...
    * Ensure link utilisation does not rise above a threshold value of 70% for any of the links
    
    * Apply optimisation heuristics to determine the topology to be implemented
...
The inter-node links have speeds of 1 or 10 Mbps, with dashed lines signifying the former, and solid lines signifying the latter; the link speeds have been scaled down by a factor of 1,000 from those in AS1221, which the model is based on, to allow the network to be replicated using readily available hardware.

The traffic generated by the hosts is selected to ensure the overall traffic load is generally classified as high, medium, or low, and the controller correspondingly selects one of the three topologies shown below (grey nodes and links are in the standby state):
...
The full implementation details can be found in Chapter 4 of the “[Dissertation.pdf](https://github.com/costa4487/Dissertation-2015/blob/master/Dissertation.pdf)” document.
